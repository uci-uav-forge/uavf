...
def beta_clusters(
    clusters: int = 3, ppc: int = 20, alpha: float = 4.0, beta: float = 4.0
) -> np.ndarray:
    ...
    for c in range(clusters):
        ...
        pts[c * ppc : (c + 1) * ppc] = (
            np.random.beta(a=alpha, b=beta, size=(ppc, 2)) + loc
        )
...
class RandomPolygon(object):
    ...
    def removable_exteriors(
        self, dt: spatial.Delaunay, points: np.ndarray, ax: plt.Axes = None
    ) -> tuple:
    ...
    def del_tri(self, dt: spatial.Delaunay, rm: int) -> spatial.Delaunay:
    ...
    def _dotself(self, x: np.ndarray) -> np.ndarray:
        return x.dot(x)

    def ar(self, M: np.ndarray) -> np.float64:
        """aspect ratio"""
    ...
    def polygon(
        self, points: np.ndarray, holes: int = 0, removals: int = 30
    ) -> nx.DiGraph():
        ...
        for _ in range(holes):
            safe, _ = self.removable_interiors(dt)
            if safe.size > 0:
                dt = self.del_tri(dt, np.random.choice(safe))
        # remove exterior tris
        n = 0
        while self.removable_exteriors(dt, points)[0] and n < removals:
            n += 1
            _, ets = self.removable_exteriors(dt, points)
            ets = sorted(ets, key=lambda x: self.ar(points[dt.simplices[x]]))
            dt = self.del_tri(dt, np.random.choice(ets))
        ...
        for s, n in zip(dt.simplices, dt.neighbors):
            if (n == -1).any():
                ...
                # not possible
                    ...
                    raise (
                        Exception(
                            "Tri identified as outside that is not on the outside!"
                        )
                    )
