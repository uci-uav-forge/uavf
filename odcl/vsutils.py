...
class VideoStreamCV(object):
    ...
    def get_img(self, undistort=False):
        try:
            if undistort:  #if undistort is true, undistort image before sending to stream
                h,w = self.img.shape[:2]
                optimal_mtx, _ = cv2.getOptimalNewCameraMatrix(self.proj_mtx, self.distortion, (w,h), 1, (w,h))
                self.img = cv2.undistort(self.img, self.proj_mtx, self.distortion, None, optimal_mtx)
            return self.img
